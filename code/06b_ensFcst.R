...
if(FALSE) {
  ...
  licePerFish_wfs <- workflow_set(
    ...
    workflow_map("tune_grid",
                 ...
                 verbose=T)
  plan(sequential)
  cat(format(Sys.time(), "%F %T"), "  Finished licePerFish tuning, advance: 1 wk", "\n")
  autoplot(licePerFish_wfs) + scale_colour_brewer(type="qual", palette="Paired")
  ggsave(glue("figs/licePerFish_ranks_1wk.png"), width=15, height=5)
  ...
  for(m in c("rmse", "rsq")) {
    ...
    map(m,
        ~rank_results(licePerFish_wfs, rank_metric=.x, select_best=TRUE) |>
          filter(.metric==.x) |>
          select(rank, .metric, mean, model, wflow_id, .config)) |>
      saveRDS(glue("out/ensembles/licePerFish_ranks_1wk_{m}.rds"))
    gc()
    ...
    saveRDS(licePerFish_best_wf, glue("out/ensembles/licePerFish_best_wf_1wk_{m}.rds"))
    saveRDS(licePerFish_best_results, glue("out/ensembles/licePerFish_best_results_1wk_{m}.rds"))
    saveRDS(licePerFish_final_fit, glue("out/ensembles/licePerFish_best_fitted_1wk_{m}.rds"))
    write_csv(licePerFish_best_preds, glue("out/ensembles/CV_ensFc-1_{m}.csv")) 
  }
...
if(TRUE) {
  ...
  liceBinary_wfs <- workflow_set(
    ...
    workflow_map("tune_grid",
                 ...
                 verbose=T)
  plan(sequential)
  cat(format(Sys.time(), "%F %T"), "  Finished liceBinary tuning, advance: 1 wk", "\n")
  autoplot(liceBinary_wfs) + scale_colour_brewer(type="qual", palette="Paired")
  ggsave(glue("figs/liceBinary_ranks_1wk.png"), width=15, height=5)
  ...
  for(m in c("roc_auc", "average_precision")) {
    map(m,
        ~rank_results(liceBinary_wfs, rank_metric=.x, select_best=TRUE) |>
          filter(.metric==.x) |>
          select(rank, .metric, mean, model, wflow_id, .config)) |>
      saveRDS(glue("out/ensembles/liceBinary_ranks_1wk_{m}.rds"))
    # Best fits
    ...
    saveRDS(liceBinary_best_wf, glue("out/ensembles/liceBinary_best_wf_1wk_{m}.rds"))
    saveRDS(liceBinary_best_results, glue("out/ensembles/liceBinary_best_results_1wk_{m}.rds"))
    saveRDS(liceBinary_final_fit, glue("out/ensembles/liceBinary_best_fitted_1wk_{m}.rds"))
    write_csv(liceBinary_best_preds, glue("out/ensembles/CV_ensFc-1_{m}.csv"))
  }
