...
for(k in (seq_along(folds))) {
  ...
  for(i in 1:nrow(sim_i)) {
    ...
    fit_ijk <- glue("out/candidates/TRT_sens/{sim}_inclPostTreat_CV-{folds[k]}_stanfit.rds")
  ...
  CV_k[[k]] <- inner_join(ensFull_df |> select(rowNum),
                          reduce(CV_i, full_join, by=join_by(rowNum)),
                          by=join_by(rowNum))
}

reduce(CV_k, bind_rows) |>
  write_csv("out/candidates/TRT_sens/CV_candidate_predictions.csv")
...
for(k in seq_along(folds)) {
  ...
  for(i in 1) {
    ...
    dat_rstan <- ensFull_df |>
      ...
      make_data_rstan_GQ(test_rows)
    fit_ijk <- glue("out/ensembles/TRT_sens/{sim}_inclPostTreat_CV-{folds[k]}_stanfit.rds")
    if(file.exists(fit_ijk)) {
      out_sim <- readRDS(fit_ijk)
    } else {
      out_sim <- stan(file="code/stan/constituent_model_GQ.stan",
...
reduce(CV_k, bind_rows) |>
  write_csv("out/ensembles/TRT_sens/CV_avg_predictions.csv")
