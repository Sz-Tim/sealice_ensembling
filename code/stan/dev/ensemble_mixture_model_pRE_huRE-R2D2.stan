...
transformed data {
  ...
  matrix[N, K_int_sims] XcInt = append_col(rep_vector(1.0, N), Xc);  // centered version of X_hu with an intercept
}
parameters {
  ...
  vector<lower=0>[K_int_sims] sd_grp_hu;  // group-level standard deviations
  matrix[K_int_sims, N_groups] z_grp_hu;  // standardized group-level effects
  cholesky_factor_corr[K_int_sims] L_grp_hu;  // cholesky factor of correlation matrix
  // parameters of the R2D2 prior
  real<lower=0,upper=1> R2D2_R2_hu;
  simplex[K_sims] R2D2_phi_hu;
}
transformed parameters {
  vector[K_sims] b_hu;  // unscaled hurdle coefficients
  ...
  matrix[N_groups, K_int_sims] r_grp_hu;
  ...
  lprior += normal_lpdf(sd_grp_hu | 0, 1)
...
model {
  ...
  if (!prior_only) {
    ...
    vector[N_Yg0] IP_ens;
    vector[N] hu;
    hu = Intercept_hu + Xc * b_hu + rows_dot_product(r_grp_hu[J_group,], XcInt);
