...
data {
  ...
  // R2D2 priors
  real<lower=0> R2D2_mean_R2_grp;  // mean of the R2 prior
  real<lower=0> R2D2_prec_R2_grp;  // precision of the R2 prior
  vector<lower=0>[K_sims] R2D2_cons_D2_grp; // concentration vector of the D2 prior
...
parameters {
  ...
  real<lower=0,upper=1> R2D2_R2_grp;
  simplex[K_sims] R2D2_phi_grp;
...
transformed parameters {
  ...
  real R2D2_tau2_grp;  // global R2D2 scale parameter
  ...
  R2D2_tau2_grp = R2D2_R2_grp / (1 - R2D2_R2_grp);
  sd_grp = scales_R2D2(R2D2_phi_grp, R2D2_tau2_grp);
  ...
  lprior += beta_lpdf(R2D2_R2_grp | R2D2_mean_R2_grp * R2D2_prec_R2_grp, (1 - R2D2_mean_R2_grp) * R2D2_prec_R2_grp);
...
model {
  ...
  target += dirichlet_lpdf(R2D2_phi_grp | R2D2_cons_D2_grp);
