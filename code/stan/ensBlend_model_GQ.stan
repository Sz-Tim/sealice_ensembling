...
data {
  ...
  array[GQ_N] int<lower=1> GQ_J_group;  // grouping indicator per observation
  matrix[GQ_N, K_sims] GQ_X;  // design matrix
  matrix[GQ_N, K_sims] GQ_Xc; // design matrix (centered)
}
transformed data {
...
generated quantities {
  ...
  vector[GQ_N] GQ_IP_ens;
  vector[GQ_N] GQ_mu;
  vector[GQ_N] GQ_hu;
  vector[GQ_N] GQ_Ypred;
  
  GQ_IP_ens = rows_dot_product(b_p[GQ_J_group,], GQ_X);
  GQ_mu = b_b0 + b_IP * GQ_IP_ens;
